...
def HyObscure(deltaX, grid_area_number, cluster_num, k_threshold, l_threshold, df_test, grid_list,
    ...
    for op in range(0, 3):
        ...
        for gg in range(0, grid_area_number):
            ...
            eng.edit('../../matlab/checkin_areanum_scenario_I/HyObscure', nargout=0)
            eng.cd('../../matlab/checkin_areanum_scenario_I', nargout=0)
...
def YGen(df_train, df_test, df_test_rec_items, cluster_num, grid_area_number, grid_list, area_grid_dict, grid_area_dict,
    ...
    for gg in range(0, grid_area_number):
        eng = matlab.engine.start_matlab()
        eng.edit('../../matlab/checkin_areanum_scenario_I/YGen', nargout=0)
        eng.cd('../../matlab/checkin_areanum_scenario_I', nargout=0)
...
def XObf(df_train, df_test, deltaX, cluster_num, grid_area_number, grid_list, area_grid_dict, pp, method):
    ...
    for gg in range(0, grid_area_number):
        eng = matlab.engine.start_matlab()
        eng.edit('../../matlab/checkin_areanum_scenario_I/XObf', nargout=0)
        eng.cd('../../matlab/checkin_areanum_scenario_I', nargout=0)
...
def PrivCheck(df_train, df_test, df_test_rec_items, grid_area_dict, area_grid_dict, grid_list,
    ...
    eng = matlab.engine.start_matlab()
    eng.edit("../../matlab/checkin_areanum_scenario_I/PrivCheck", nargout=0)
    eng.cd('../../matlab/checkin_areanum_scenario_I', nargout=0)
...
def differential_privacy(df_train, df_test, grid_area_dict, df_test_rec_items, beta):
    df_train = funcs.update_grid_group(df_train, grid_area_dict)
...
def Frapp(df_train, df_test, grid_area_dict, df_test_rec_items, gamma, pp):
    df_train = funcs.update_grid_group(df_train, grid_area_dict)
