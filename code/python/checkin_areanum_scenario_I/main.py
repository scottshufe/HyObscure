...
if __name__ == '__main__':
    ...
    df_item_gridGroup_uid = df[cols_change]
    ...
    for grid_area_number in grid_area_number_list:
        ...
        avg_acc_ori_rf = np.mean(np.array(results['ori_acc_rf']))
        avg_acc_obf_rf = np.mean(np.array(results['obf_acc_rf']))
        avg_acc_ori_xgb = np.mean(np.array(results['ori_acc_xgb']))
        avg_acc_obf_xgb = np.mean(np.array(results['obf_acc_xgb']))
        avg_rec_ori = np.mean(np.array(results['rec_ori']))
        avg_rec_obf = np.mean(np.array(results['rec_obf']))
        
        with open('checkin_areanum_experiments_results', 'a') as result_file:
            result_file.write("%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n"%(method, deltaX, grid_area_number,
                        avg_acc_ori_rf, avg_acc_obf_rf, avg_acc_ori_xgb, avg_acc_obf_xgb, avg_rec_ori, avg_rec_obf))
            result_file.flush()