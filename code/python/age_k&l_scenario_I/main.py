    ...
    for l_threshold in l_threshold_list:
        for k_threshold in k_threshold_list:
            ...
            for r in available_seeds:
                ...
                results['ori_age_rf'].append(np.mean(mae_oris_rf))
                results['obf_age_rf'].append(np.mean(mae_obfs_xgb))
                results['ori_age_xgb'].append(np.mean(mae_oris_xgb))
                results['obf_age_xgb'].append(np.mean(mae_obfs_xgb))
            ...
            with open("HyObscure_age_k&l_scenario_I_results", 'a') as file_out_overall:
                file_out_overall.write(
                    "%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n" % (cluster_num, deltaX, k_threshold, l_threshold,
                                                                  avg_mae_ori_rf, avg_mae_obf_rf,
                                                                  avg_mae_ori_xgb, avg_mae_obf_xgb,
