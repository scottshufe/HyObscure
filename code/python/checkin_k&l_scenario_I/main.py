...
def get_JSD_PGY(df_cluster, area_grid_dict):
    ...
    # with open('grid_group_file', 'a') as ggf:
    #     ggf.write('\n')
    #     ggf.flush()
    for gg in range(grid_area_number):
        grid_list_gg = area_grid_dict[gg]
        # with open('grid_group_file', 'a') as ggf:
        #     ggf.write("%s\t" %len(grid_list_gg))
        #     ggf.flush()
        grid_list_gg.sort()
    ...
    for l_threshold in l_threshold_list:
        for k_threshold in k_threshold_list:
            ...
            for r in range(20):
                ...
                items = list(df_item_gridGroup_uid)
                items.remove('grid')
                ...
                test_idx_list = []
                for i in grid_list:
                    df_c = df_train_items.loc[df_train_items['grid']==i]
                ...
                cluster_flag = 0
                for i in df_test.cluster.value_counts():
                    if i == 0:
                        cluster_flag = 1
                if cluster_flag == 1:
                    continue
                ...
                for op in range(0, 5):
                    ## compute JSD and pgy
                    JSD_Mat, pgy, JSD_Mat_dict, pgy_dict = get_JSD_PGY(df_test, area_grid_dict)
                    print('op:', op)
                    grid_xpgg_dict = {}
                    ## compute xpgg
                    for gg in range(0, grid_area_number):
                        eng = matlab.engine.start_matlab()
                        eng.edit('../../matlab/checkin_k&l_scenario_I/HyObscure', nargout=0)
                        eng.cd('../../matlab/checkin_k&l_scenario_I', nargout=0)
                            ...
                            for line in area_lines:
                                ...
                                for i in range(1, change_range + 1):
                                    ...
                                    if k_adjust >= k_threshold:
                                        ...
                                        df_test_new = update_grid_group(df_test, grid_area_dict_cur)
                                        # try:
                                        new_JSD_Mat, new_pgy, new_JSD_Mat_dict, new_pgy_dict = get_JSD_PGY(df_test_new,
                                                                                                           area_grid_dict_cur)
                                        ...
                                        if new_mean_Privacy < min_mean_Privacy and new_mean_Utility < min_mean_Utility:
                                            ...
                                            area_grid_rowcol_dict, area_grid_colrow_dict = update_rowcol_colrow_dict(area_grid_dict)
                ...
                for i in range(25):
                    map_ori, map_obf = recommendation(X_obf_dict[i], X_ori, df_test_rec_items)
                    ...
                    acc_ori_rf = accuracy_score(df_x_y, y_pred_ori_rf)
                    acc_obf_rf = accuracy_score(df_x_y, y_pred_obf_rf)
                    ...
                    acc_ori_xgb = accuracy_score(df_x_y, y_pred_ori_xgb)
                    acc_obf_xgb = accuracy_score(df_x_y, y_pred_obf_xgb)
                ...
                results['ori_acc_rf'].append(np.mean(acc_oris_rf))
                results['obf_acc_rf'].append(np.mean(acc_obfs_rf))
                results['ori_acc_xgb'].append(np.mean(acc_oris_xgb))
                results['obf_acc_xgb'].append(np.mean(acc_obfs_xgb))
                results['rec_ori'].append(np.mean(rec_oris))
                results['rec_obf'].append(np.mean(rec_obfs))

            avg_mae_ori_rf = np.mean(np.array(results['ori_acc_rf']))
            avg_mae_obf_rf = np.mean(np.array(results['obf_acc_rf']))
            avg_mae_ori_xgb = np.mean(np.array(results['ori_acc_xgb']))
            avg_mae_obf_xgb = np.mean(np.array(results['obf_acc_xgb']))
            avg_rec_ori = np.mean(np.array(results['rec_ori']))
            avg_rec_obf = np.mean(np.array(results['rec_obf']))

            with open('HyObscure_checkin_k&l_scenario_I_results', 'a') as file_out_overall:
