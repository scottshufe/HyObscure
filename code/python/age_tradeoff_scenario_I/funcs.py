...
def getKNearestUsers(sim_mat, uidIndexDict, indexUidDict, uid):
    KNNuids = []
    idx = uidIndexDict[uid]
    simlist = sim_mat[idx]
    idx_sim_dict = {}
    for i in range(len(simlist)):
        idx_sim_dict[i] = simlist[i]
    sorted_tuple_list = sorted(idx_sim_dict.items(), key=lambda x: x[1], reverse=True)
    ...
    for i in idxs:
        KNNuids.append(indexUidDict[i])
    got_list = []
    for i in range(len(KNNuids)):
        got_list.append((KNNuids[i], KNNsims[i]))

    return got_list


def get_sim_matrix(df):
    sim_matrix = 1 - squareform(pdist(df.values, metric='jaccard'))
    return sim_matrix


def get_user_sim_dict(df, uid_index_dict, index_uid_dict):
    sim_matrix = get_sim_matrix(df)
    uid_and_simDict = {}
    for uid in df.index:
        uid_and_simDict[uid] = getKNearestUsers(sim_matrix, uid_index_dict, index_uid_dict, uid)
    return uid_and_simDict


def get_user_item_dict(df, books_col):
    userItemsDict = {}
    values = df.values
    for i in range(values.shape[0]):
        userItemsDict[df.index[i]] = []
        for j in range(values.shape[1]):
            if values[i][j] == 1:
                userItemsDict[df.index[i]].append(books_col[j])
    return userItemsDict
...
def cal_distortion_budget(X_ori, X_obf):
    ...
    cos_dist = np.mean(np.diag(cosine_distances(df_x_ori_items, df_x_obf_items)))
    euclid_dist = np.mean(np.diag(euclidean_distances(df_x_ori_items, df_x_obf_items)))
