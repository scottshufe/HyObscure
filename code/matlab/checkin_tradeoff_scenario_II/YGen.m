...
function FX = YGen(deltaX, gg)
%function X = ageGroupPrivacyObf()
pgy_input = csvread(strcat('../../python/checkin_tradeoff_scenario_II/tmp/pgy_check_in_',num2str(gg),'_YGen.csv'));
JSD_Mat_input = csvread(strcat('../../python/checkin_tradeoff_scenario_II/tmp/JSDM_check_in_',num2str(gg),'_YGen.csv'));
size(pgy_input)
size(JSD_Mat_input)
