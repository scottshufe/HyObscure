...
pgy_input = csvread('../../python/checkin_clusternum_scenario_I/tmp/pgy_check_in_privcheck.csv');
load('../../python/checkin_clusternum_scenario_I/tmp/JSDM_girdGroup_privcheck')
%JSD_Mat_input = load('JSDM_ageGroup.csv');
size(pgy_input)
size(JSD_Mat_input_Yang_trueTrain)

% set error bound
%deltaX = 0.5;

% learn obfuscation matrix
[ X, v, DB ] = pripmapping(pgy_input', JSD_Mat_input_Yang_trueTrain, deltaX);
