...
pgy_input = csvread('../../python/age_clusternum_scenario_II/tmp/pgy_ageGroup_privcheck.csv');
load ('../../python/age_clusternum_scenario_II/tmp/JSDM_ageGroup_privcheck')
%JSD_Mat_input = load('JSDM_ageGroup.csv');
size(pgy_input)
size(JSD_Mat_input_origin)

% set error bound
%deltaX = 0.5;

% learn obfuscation matrix
[ X, v, DB ] = pripmapping(pgy_input', JSD_Mat_input_origin, deltaX);
